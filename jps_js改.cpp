			  ...
			  const Location & goal)
{
	...
	if (dx != 0 && dy != 0)
	{
		...
		if (jump(grid,
				 ...
				 , current, goal) != NoneLoc || jump(grid,
													 {
													 current.x, current.y + dy}
													 , current, goal) != NoneLoc)
		{
			return current;
		}
	}
	...
	else
	{
		if (dx != 0)
		{						// moving along x
			if (grid.isWalkableAt(current.x + dx, current.y)&&((grid.isWalkableAt(current.x + dx, current.y + 1)
				 && !grid.isWalkableAt(current.x, current.y + 1))
				|| (grid.isWalkableAt(current.x + dx, current.y - 1)
					&& !grid.isWalkableAt(current.x, current.y - 1))))
			{
				return current;
			}
		...
		{
			if (grid.isWalkableAt(current.x, current.y + dy)&&((grid.isWalkableAt(current.x + 1, current.y + dy)
				 && !grid.isWalkableAt(current.x + 1, current.y))
				|| (grid.isWalkableAt(current.x - 1, current.y + dy)
					&& !grid.isWalkableAt(current.x - 1, current.y))))
			{
				return current;
			}
	...
	if (grid.isWalkableAt(current.x + dx, current.y)
		|| grid.isWalkableAt(current.x, current.y + dy))
	{
		return jump(grid,
					{
	...
	while (!openList.empty())
	{
		...
		for (int i = 0, l = neighbors.size(); i < l; ++i)
		{
			...
			if (jumpPoint != NoneLoc)
			{
				...
				if (cost_so_fa.find(jumpPoint) == cost_so_fa.end()
					|| new_cost < cost_so_fa[jumpPoint])
				{
					cout<<"jumpPoint:"<<jumpPoint<<endl;
					cost_so_fa[jumpPoint] = new_cost;
					came_from[jumpPoint] = current;
					openList.emplace(new_cost + euclidean(jumpPoint, goal),
									 jumpPoint);
